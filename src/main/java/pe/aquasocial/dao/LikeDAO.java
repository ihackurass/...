...
public class LikeDAO implements ILikeDAO {
    ...
    public int contarLikesRecibidosUsuario(int idUsuario) {
        String sql = "SELECT COUNT(*) FROM likes l "
                + "JOIN publicaciones p ON l.id_publicacion = p.id_publicacion "
                + "WHERE p.id_usuario = ?";
        ...
        try (Connection conn = Conexion.getConexion(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                System.out.println("❤️ Usuario " + idUsuario + " ha recibido " + count + " likes en sus publicaciones");
                return count;
            }

        } catch (SQLException e) {
            System.err.println("❌ Error al contar likes recibidos por usuario: " + e.getMessage());
            e.printStackTrace();
        }
        return 0;
    }
    ...
    public int contarLikesDadosUsuario(int idUsuario) {
        String sql = "SELECT COUNT(*) FROM likes WHERE id_usuario = ?";
        ...
        try (Connection conn = Conexion.getConexion(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                int count = rs.getInt(1);
                System.out.println("👍 Usuario " + idUsuario + " ha dado " + count + " likes");
                return count;
            }

        } catch (SQLException e) {
            System.err.println("❌ Error al contar likes dados por usuario: " + e.getMessage());
            e.printStackTrace();
        }
        return 0;
    }
